...
CI/CD STATUS BADGES
-------------------
Heads-up: the CI/CD GitHub Actions workflows described below are PLANNED but
are NOT yet committed to this repo (there is no .github/workflows/ directory
yet). The two badges below will only render a real status once you:
  1. add .github/workflows/ci.yml and image-release.yml, and
  2. push this project to its own GitHub repo,
then replace YOUR_USERNAME/kubeops-gitops with your real repo path.
Until then they will show "no status".
-->

[![CI](https://img.shields.io/github/actions/workflow/status/YOUR_USERNAME/kubeops-gitops/ci.yml?branch=main&label=CI)](https://github.com/YOUR_USERNAME/kubeops-gitops/actions/workflows/ci.yml)
[![Image Release](https://img.shields.io/github/actions/workflow/status/YOUR_USERNAME/kubeops-gitops/image-release.yml?branch=main&label=image-release)](https://github.com/YOUR_USERNAME/kubeops-gitops/actions/workflows/image-release.yml)
...
flowchart TD
    ...
    subgraph CI["CI/CD — GitHub Actions (planned, not yet committed)"]
...
| CI/CD (planned)| GitHub Actions, GHCR, Trivy image scan                               |
...
> **Honest status:** the CI/CD workflows are **designed but not yet committed** —
> there is no `.github/workflows/` directory in the repo at the time of writing.
> The badges at the top will activate once the workflows exist and the project is
> pushed to its own repo. The intended design is:

- **`ci.yml`** (on push / pull_request): checkout → set up Python 3.12 → install
  dev deps → `ruff check` → `black --check` → `isort --check-only` → `pytest` →
  `bandit` → `pip-audit` → Docker build → Trivy image scan.
- **`image-release.yml`** (on push to `main`): checkout → log in to GHCR → build
  image → tag with `latest` and the commit SHA → push to
  `ghcr.io/<owner>/kubeops` → Trivy scan.

This project uses **GitOps (Argo CD)** for deployment, so CI/CD stops at
building/publishing the image — it does not push directly to a cluster.
...
via bandit, pip-audit, and (planned) Trivy. Full write-up and the honest list of
...
- Argo CD manifests are provided, but **Argo CD is installed later** — it is not
  bundled here.
- **CI/CD workflows are documented but not yet committed** to the repo.
...
- Commit the GitHub Actions workflows and wire up GHCR publishing.
